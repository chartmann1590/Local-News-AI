...
- **Radar Not Updating:** Radar now updates immediately when location is changed. If it doesn't update, refresh the page or check that the location was successfully saved in Settings.

## Unit Preferences

- **Temperature/Wind Speed Units:** Units should always respect your settings. If they default to Celsius/kmh, ensure your preferences are saved in Settings. The database will initialize with Fahrenheit/mph defaults if no settings exist.

## Article Rewrites

- **Raw Content Showing:** Articles may temporarily show raw content if a rewrite hasn't completed yet. A warning will be displayed indicating the article will be rewritten on the next scheduled run.
- **Rewrite Not Happening:** Rewrites run automatically on scheduled harvest cycles. Articles missing rewrites are automatically included in the next rewrite cycle. Check the scheduler logs to see if rewrites are running.
